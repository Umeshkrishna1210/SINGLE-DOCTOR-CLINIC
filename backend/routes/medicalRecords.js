...
// POST /medical-records/ - Patient Upload (Uses medical_records)
// This route remains unchanged and correctly uses medical_records
router.post("/", authMiddleware, (req, res) => {
  ...
  upload(req, res, function (err) {
    ...
    console.log("Data prepared for DB insert (Patient Upload - medical_records):", { patient_id, problem, previousMedications, medicalHistory, prescriptions: prescriptionsJsonString, labReports: labReportsJsonString });
    
    // Simple INSERT - each upload creates a new separate record
    const insertQuery = `INSERT INTO medical_records (patient_id, problem, previous_medications, medical_history, prescriptions, lab_reports) VALUES (?, ?, ?, ?, ?, ?)`;
    ...
    db.query( insertQuery, [ patient_id, problem, previousMedications, medicalHistory, prescriptionsJsonString, labReportsJsonString ], (dbError, result) => { 
        if (dbError) { /* ... error handling & cleanup ... */ 
            console.error("Database insert error (Patient Upload - medical_records):", dbError);
            if (req.files) { /* ... cleanup logic ... */ }
            return res.status(500).json({ error: "Database operation failed." });
        }
        console.log(`Successfully inserted record ID: ${result.insertId} into medical_records`); 
    ...
    db.query(sql, [patientId], (err, rows) => { 
        ...
        try { 
            // Check if prescription is already an object/array (driver parsed it)
             if (record.prescriptions && typeof record.prescriptions === 'object') {
                medicationList = Array.isArray(record.prescriptions) ? record.prescriptions : [];
             } 
             // Check if it's a non-empty string, then try parsing
             else if (record.prescriptions && typeof record.prescriptions === 'string') {
                medicationList = JSON.parse(record.prescriptions);
...
router.get('/myprescriptions', authMiddleware, (req, res) => { // Removed async
    ...
    const sql = `SELECT mr.id, mr.doctor_id, u.name AS doctorName, mr.diagnosis, mr.prescriptions FROM medical_records mr JOIN users u ON mr.doctor_id = u.id WHERE mr.patient_id = ? LIMIT 1`; // Limit 1
